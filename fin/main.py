...
from bluetooth_service import ArduinoBluetooth, BluetoothConnectionError
...
def main(page: ft.Page):
    ...
    def show_bluetooth_screen():
        ...
        continue_button = primary_button(
            "Продолжить",
            lambda e: show_tea_form(),
            disabled=True,
        )

        def skip_bluetooth(e):
            status.value = "Режим отладки: подключение пропущено"
            status.color = ft.Colors.ORANGE
            page.update()
            show_tea_form()

        def refresh_ports(e):
            updated_ports = arduino.list_ports()
            port_dropdown.options = [
                ft.dropdown.Option(port) for port in updated_ports
            ]
            if updated_ports:
                port_dropdown.value = updated_ports[0]
            else:
                port_dropdown.value = None
            status.value = (
                "Порты обновлены"
                if updated_ports
                else "Bluetooth-порты не найдены. Сопрягите HC-05/HC-06 в Windows."
            )
            status.color = GREEN_DARK if updated_ports else ft.Colors.RED
            page.update()

        def connect(e):
            if not port_dropdown.value:
                status.value = "Выберите COM-порт"
                status.color = ft.Colors.RED
                page.update()
                return

            try:
                arduino.connect(port_dropdown.value)
            except BluetoothConnectionError as exc:
                status.value = str(exc)
                status.color = ft.Colors.RED
                continue_button.disabled = True
            else:
                status.value = f"Подключено: {arduino.port}"
                status.color = GREEN_DARK
                continue_button.disabled = False
            page.update()

        def disconnect(e):
            arduino.disconnect()
            status.value = "Подключение разорвано"
            status.color = ft.Colors.RED
            continue_button.disabled = True
            page.update()

        hint = (
            "HC-05/HC-06 после сопряжения появляется как COM-порт."
            if ArduinoBluetooth.is_available()
            else "Установите pyserial: pip install pyserial"
        )

        show_screen(
            screen_title(f"{user_name}, подключите Arduino"),
            body_text(
                "Выберите Bluetooth-модуль Arduino и установите соединение.",
            ),
            body_text(hint, size=13, secondary=True),
            port_dropdown,
            primary_button("Обновить порты", refresh_ports),
            primary_button("Подключиться", connect),
            primary_button("Отключиться", disconnect),
            status,
            continue_button,
            ft.TextButton(
                content=ft.Text("Пропустить (отладка)", color=GREEN_DARK),
                on_click=skip_bluetooth,
            ),
    ...
    def show_mix_screen():
        ...
        def send_to_arduino(e):
            ...
            try:
                arduino.send_mix(last_grams, round(sugar_slider.value, 1))
            except BluetoothConnectionError as exc:
                send_status.value = str(exc)
                send_status.color = ft.Colors.RED
            else:
                send_status.value = "Данные отправлены на Arduino"
